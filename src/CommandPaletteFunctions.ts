...
export function commandPaletteFilterEntries(entries: CommandEntry[], filter: string): CommandEntry[] {
  const lowerFilter = filter.toLowerCase();
  return entries.filter( (entry) => entry.label.toLowerCase().includes(lowerFilter) );
}

export function commandPaletteFormatEntries(entries: CommandEntry[], selectedId: string, filterInputValue: string): string {
    return (filterInputValue.trim() === "" ? commandPaletteFormatEntriesWithGroups : commandPaletteFormatEntriesAsList)(entries, selectedId);
}

function commandPaletteFormatEntriesAsList(entries: CommandEntry[], selectedId: string): string {
  return entries.map( (entry) => commandPaletteFormatEntry(entry, entry.id === selectedId) ).join("");
}

function commandPaletteFormatEntriesWithGroups(entries: CommandEntry[], selectedId: string): string {
...
function commandPaletteFormatEntry(entry: CommandEntry, selected: boolean, extraClassString = ""): string {
